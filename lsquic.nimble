...
task test, "Run tests":
  when defined(windows):
    exec "nim c --mm:refc -d:nimDebugDlOpen -d:fast=true --threads:on tests/test_all.nim"
  else:
    exec "nim c --mm:refc -d:fast=true --threads:on tests/test_all.nim"
  exec "./tests/test_all --output-level=VERBOSE"

task test_release, "Run tests - release":
  when defined(windows):
    exec "nim c -d:release --mm:refc -d:fast=true -d:nimDebugDlOpen --threads:on tests/test_all.nim"
  else:
    exec "nim c -d:release --mm:refc -d:fast=true --threads:on tests/test_all.nim"
  exec "./tests/test_all --output-level=VERBOSE"
